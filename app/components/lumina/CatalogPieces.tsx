...
/**
 * Shared catalog grid section. Used by the combined Shop All page
 * (/collections/all) and the two gendered pages (/collections/male,
 * /collections/female). Renders an Eyebrow + headline above a 1→5
 * column grid of CatalogCards with the savings engine baked in.
 *
 * Hoisted out of collections.all.tsx so the gendered routes don't have
 * to duplicate the layout or drift out of sync.
 */
 ...
 * Lives here so both the combined catalog and the gendered pages
 * can decide independently whether to surface it.
 */
export function HowLuminaCompares() {
  const rows: Array<{
    feature: string;
    lumina: string;
    typical: string;
